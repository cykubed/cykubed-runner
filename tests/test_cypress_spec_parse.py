...
def test_get_specs_new_style(cypress_fixturedir):
    specs = set(get_specs(os.path.join(cypress_fixturedir, 'new-style-cypress')))
...
def test_get_specs_legacy_default(cypress_fixturedir):
    specs = set(get_specs(os.path.join(cypress_fixturedir, 'legacy-cypress-defaults')))
...
def test_get_specs_legacy_override(cypress_fixturedir):
    specs = set(get_specs(os.path.join(cypress_fixturedir, 'legacy-cypress-override')))
    assert specs == {'cypress/e2e/stuff/test1.spec.ts'}


def test_default_specs(cypress_fixturedir):
    specs = set(get_specs(os.path.join(cypress_fixturedir, 'nextjs')))
